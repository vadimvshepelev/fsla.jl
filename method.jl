...
function calc(pr::Dict, eos::eos_ideal, fld::Field_primitive)::Int8  
	...
	w, w_new = fld.w, fld.w_new
    g::Vector{Float64} = [(visc_const * w[i][1] * (w[i+1][2] - w[i][2])) for i in 1:N] * [0.]
    w1=w.copy()
    while it_num < 31
        w1=w_new.copy()
        for i = 1:N+1       
		...
		for i=1:N+1 
			w_new[i][1] = 1.0 / (1.0/w[i][0] + 0.5*dt/dm *(w_new[i+1][2] + w[i+1][2] - w_new[i][2] - w[i][2]))        
...
function calcdt(pr::Dict, eos::eos_ideal, fld::Field_primitive) 
	...
	dt = (x[2] - x[1]) / (abs(w[1][2]) + c_local)
	for i=2:N+1
		c_local, dt = getc(eos, w[i][1], w[i][3]), min(dt, (x[2] - x[1]) / (abs(w[i][2]) + c_local))
	end
	dt
end
